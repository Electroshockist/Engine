...
bool World1::OnCreate(){
	IMGUI_CHECKVERSION();
	ImGui::CreateContext();
	ImGui_ImplSDL2_InitForOpenGL(EngineMain::GetInstance()->w->window, EngineMain::GetInstance()->w->context);
	...
	model = new Model("./Resources/Models/untitled.obj", "./Resources/Materials/Dice.mtl", ShaderManager::GetInstance()->getShader("basicShader"));
...
bool World1::Render(){
	...
	ImGui_ImplSDL2_NewFrame(EngineMain::GetInstance()->w->window);
	ImGui::NewFrame();
